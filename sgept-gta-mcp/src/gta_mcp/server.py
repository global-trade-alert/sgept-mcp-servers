...
async def gta_get_intervention(
    ...
    """Fetch full details for one or multiple GTA interventions by ID, with optional field projection.

    USE THIS TOOL when you need the full intervention text, source documents, or complete metadata.
    ``gta_search_interventions`` returns summaries; ``gta_count_interventions`` returns aggregate counts
    — neither provides the full description or source text.

    **Single fetch (back-compat):** pass ``intervention_id=N``.
    **Batch fetch:** pass ``intervention_ids=[N, M, ...]`` to retrieve multiple records in one call,
    with partial-success: valid IDs return records, invalid IDs return error metadata.

    **Field projection** via ``show_keys`` limits which fields are included per record.
    Use this to stay within the token budget on broad queries.
    Available field names:
      ...
      affected_products, intervention_description, state_act_source,
      intervention_url, state_act_url, is_official_source.
    Pass ``["*"]`` for all fields (default full record).

    Args:
        intervention_id: Single GTA intervention ID. Mutually exclusive with intervention_ids.
        intervention_ids: List of GTA intervention IDs for batch fetch. Mutually exclusive with intervention_id.
        show_keys: Field projection list. Unknown names are silently dropped.
        response_format: 'markdown' (default) or 'json'.

    Examples:
        - "Show me the text of intervention 138295" → intervention_id=138295
        - "Fetch details for interventions 138295, 138296, 138297" → intervention_ids=[138295, 138296, 138297]
        - "Get title and evaluation for these 5 IDs" → intervention_ids=[...], show_keys=["intervention_id","state_act_title","gta_evaluation"]
    """
    params = GTAGetInterventionInput(
    ...
    try:
        ...
        if params.intervention_ids is not None:
            # Batch path
            batch = await client.get_interventions_batch(
                params.intervention_ids,
                show_keys=params.show_keys,
            )
            results = batch["results"]
            errors = batch["errors"]
            if not results and errors:
                ids_str = ", ".join(str(e["intervention_id"]) for e in errors)
                raise ToolError(f"None of the requested interventions were found: {ids_str}")
            if params.response_format == ResponseFormat.MARKDOWN:
                return format_interventions_batch_markdown(results, errors, show_keys=params.show_keys)
            else:
                return format_interventions_batch_json(results, errors, show_keys=params.show_keys)
        else:
        ...
        if "not found" in str(e):
            id_repr = params.intervention_id or params.intervention_ids
            raise ToolError(f"Intervention {id_repr} not found in GTA database.")
