"""Run all 20 sample prompts against the live GTA API — corrected version."""
...
async def search(params, limit=50):
    ...
    if '-date_announced' not in json.dumps(body):
        body['sorting'] = '-date_announced'
...
async def count(params, count_by, count_variable='intervention_id'):
    """Execute a count query using the GTAAPIClient directly."""
    ...
    return 200, data, messages, filters


def extract_titles(data, max_n=3):
    """Extract first N intervention titles from v2 search results."""
    if not isinstance(data, list):
        return []
    titles = []
    for item in data[:max_n]:
        title = item.get('state_act_title', item.get('title', 'No title'))
        titles.append(title)
    return titles


def extract_count_summary(data, max_n=10):
    """Extract count groups from count results."""
    if isinstance(data, list):
        return data[:max_n]
    elif isinstance(data, dict):
        return data.get('results', [])[:max_n]
    return []


# Define all 20 prompts with corrected params
PROMPTS = [
    {
        "id": 1,
    ...
    },
    {
        ...
        "params": {
            "affected_jurisdictions": ["CHN"],
            "query": "steel",
            "intervention_types": ["Anti-dumping"],
            "date_announced_gte": "2020-01-01",
        },
        "note": "Original 'steel anti-dumping' phrase returned 0. Split into query='steel' + intervention_type=Anti-dumping."
    },
    {
        "id": 14,
        ...
        "params": {
            "query": "solar",
            "intervention_types": ["Safeguard"],
            "is_in_force": True,
        },
        "note": "Original 'solar panel safeguard' phrase returned 0. Split into query='solar' + intervention_type=Safeguard."
    },
    {
        "id": 15,
        ...
        "params": {
            ...
            "intervention_types": ["Import licensing requirement"],
        },
        "note": "Original 'pharmaceutical import licen' returned 0. Split into query='pharmaceutical' + intervention_type."
    },
    {
        "id": 16,
    ...
    for p in PROMPTS:
        ...
        try:
            if query_type == "search":
                ...
                count_by = p.get("count_by", ["date_announced_year"])
